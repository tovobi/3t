...
export function DataObjectParser($data) {
	this._data = $data || {};
}
		...
		for (var name in $data) {
			...
			// check if current name is an array
				...
				result = result.concat(
					subArray.map(createMapHandler(modifiedName, $data))
				);
			}
			...
			//check if current name is an object
				...
				result = result.concat(
					subObject.map(createMapHandler(modifiedName, $data))
				);
			}
		}
		return result;
